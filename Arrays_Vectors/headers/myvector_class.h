#ifndef __VectorCLASS_H__
#define __VectorCLASS_H__
...
public:
    ...
    Vector(const uint32_t size, const int value)
    {
        actual_size = size;
    ...
    // Deconstructor that clears Vector when main() goes out of scope
    ~Vector()
    {
        Clear();
    }
