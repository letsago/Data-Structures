...
class OrderedMap
{
  ...
  public:
    ...
    V& at(const K& key) { return *((*(m_btree.find(KeyValuePair(key)))).m_pValue); }

    V& operator[](const K& key) { return at(key); }

    const V& at(const K& key) const { return *((*(m_btree.find(KeyValuePair(key)))).m_pValue); }

    const V& operator[](const K& key) const { return at(key); }

    bool contains(const K& key) const { return m_btree.contains(KeyValuePair(key)); }
