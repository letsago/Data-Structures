...
#define MEMORY_LEAK(result, starting_leaks) { \
    bool comp = (getTracker().NumMemoryLeaks() == starting_leaks) & (starting_leaks != 0xFFFFFFFF); \
...
#define ADD_TEST(test, ...) { \
    ...
    int _staring_leaks = getTracker().NumMemoryLeaks(); \
...
public:
    ...
    {
        return m_state ? m_numAllocations : 0xFFFFFFFF;
    }
